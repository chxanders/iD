...
<?php
	...
	if ($aCMDResult['create-functions'] || $aCMDResult['all'])
	{
		echo "Functions\n";
		$bDidSomething = true;
		if (!file_exists('/usr/lib/nominatim.so')) fail("nominatim module not built");
		$sTemplate = file_get_contents(CONST_BasePath.'/sql/functions.sql');
		$sTemplate = str_replace('{modulepath}', '/usr/lib/', $sTemplate);
	...
	if ($aCMDResult['index'] || $aCMDResult['all'])
	{
		...
		$sBaseCmd = '/usr/bin/nominatim -i -d '.$aDSNInfo['database'].' -t '.$iInstances.$sOutputFile;
